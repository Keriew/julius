...
static void prepare_advisor_image_ids(void)
{
    ...
    for (int i = 0; i < ADVISOR_MAX; i++) {
        if (i == (ADVISOR_HOUSING - 1)) {
            reduce = 1;
            advisor_image_ids[0][ADVISOR_HOUSING - 1] = assets_get_image_id("MSTVD", "UI_Elements",
                "Housing Advisor Button");
            advisor_image_ids[1][ADVISOR_HOUSING - 1] = assets_get_image_id("MSTVD", "UI_Elements",
