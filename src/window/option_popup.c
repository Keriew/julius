    ...
    void (*close_func)(int selection), int original_option, int price, option_menu_row_size row_size)
{
    ...
    if (!border_image_ids[0]) {
        border_image_ids[0] = assets_get_image_id("Areldir", "Econ_Logistics", "Policy Selection Borders");
        border_image_ids[1] = assets_get_image_id("Areldir", "UI_Elements", "Monument Mod Selection Borders");
    }
    return 1;
}
