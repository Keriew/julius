                ...
                void (*close_func)(int selection), int show_cancel_button)
{
    ...
    data.base_border_image_id = assets_get_image_id(assets_get_group_id("Areldir", "Caravanserai"),
...
void window_action_popup_show(int title, int prompt, option_menu_item *options,
                              void (*close_func)(int selection), int show_cancel_button)
{
    if (init(title, prompt, options[0], options[1], options[2], close_func, show_cancel_button)) {
        window_type window = {
                WINDOW_POPUP_DIALOG,
                draw_background,
                draw_foreground,
