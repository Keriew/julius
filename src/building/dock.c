...
int building_dock_accepts_ship(int ship_id, int dock_id)
{
    ...
    if (figure_trader_ship_docked_once(f, dock_id)) {
      return 0;
    }
...
int building_dock_get_free_destination(int ship_id, map_point *tile)
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (!dock->data.dock.trade_ship_id) {
            if (ship->action_state == FIGURE_ACTION_114_TRADE_SHIP_ANCHORED && ship->destination_building_id == dock_id) {
                importing_dock_id = exporting_dock_id = dock_id;
                break;
            } else if (ship->action_state == FIGURE_ACTION_110_TRADE_SHIP_CREATED) {
                importing_dock_id = dock_id;
                break;
            }
            else if (building_dock_can_import_from_ship(dock, ship_id)) {
                importing_dock_id = dock_id;
                // prioritize imports
                break;
        ...
        } else if (dock->data.dock.trade_ship_id == ship_id) {
