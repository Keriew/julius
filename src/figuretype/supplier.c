...
void figure_supplier_action(figure *f)
{
    ...
    if (f->type == FIGURE_MESS_HALL_SUPPLIER) {
        ...
        if (f->action_state == FIGURE_ACTION_149_CORPSE) {
            f->image_id = assets_get_image_id("Areldir", "Entertainment", "Barkeep Death 01") +
                figure_image_corpse_offset(f);
        } else {
            f->image_id = assets_get_image_id("Areldir", "Entertainment", "Barkeep NE 01") +
        ...
        if (f->action_state == FIGURE_ACTION_149_CORPSE) {
            ...
            f->image_id = assets_get_image_id("Areldir", "Slave_Walker", "Slave NE 01") +
                dir * 12 + f->image_offset;
        }
    } else {
...
void figure_delivery_boy_action(figure *f)
{
    ...
    if (f->type == FIGURE_MESS_HALL_COLLECTOR) {
        if (f->action_state == FIGURE_ACTION_149_CORPSE) {
            f->image_id = assets_get_image_id("Areldir", "Mess_Hall", "M Hall death 01") +
                figure_image_corpse_offset(f);
        } else {
            f->image_id = assets_get_image_id("Areldir", "Mess_Hall", "M Hall NE 01") +
                dir * 12 + f->image_offset;
        }
    } else if (f->type == FIGURE_CARAVANSERAI_COLLECTOR) {
        if (f->action_state == FIGURE_ACTION_149_CORPSE) {
            f->image_id = assets_get_image_id("Areldir", "Slave_Walker", "Slave death 01") + figure_image_corpse_offset(f);
        } else {
            f->image_id = assets_get_image_id("Areldir", "Slave_Walker", "Slave NE 01")
                + dir * 12 + f->image_offset;
        }
